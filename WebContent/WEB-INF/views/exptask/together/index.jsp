...
$(function() {
	$('#mydatagrid').datagrid({
		fitColumns : true,
		url : "<c:url value='/expqixinzhuan/list'/>",
		method : "POST",
		pageSize :10, 
		pageList : [10,20,50],
		columns : [[
					    ...
					    { "field": 'expCountGold',"title" : '活动总资产',width:$(this).width() * 0.2},
					    { "field": 'userMaxGold',"title" : '个人应得金币上限',width:$(this).width() * 0.2},
...
function updataSeqNum(){
	...
	$.post(SITE_BASE_PATH+'expqixinzhuan/updataSeqNum?', {expId: rows[0].expId,seq:seq},function(data){
		if(data.message=="1"){
...
function doAdd(){
	window.location.href="<c:url value='/expqixinzhuan/jumpAdd'/>";
}
function doUpdate(){
	var rows=$("#mydatagrid").datagrid("getSelections");
	if(rows.length!=1){
	...
	$.post(SITE_BASE_PATH+'expqixinzhuan/isBeg?', {expId: rows[0].expId},function(data){
		if(data.message=="0"){
			
			window.location.href="<c:url value='/expqixinzhuan/jumpEdit?id="+rows[0].expId+"'/>";
...
function doDelete(){
	...
	$.post(SITE_BASE_PATH+'expqixinzhuan/delete?', {id: ids.join(',')},function(data){
			if(data.result > 0){
...
function doSetting(){
	...
	$.post(SITE_BASE_PATH+'expqixinzhuan/isBeg?', {expId: rows[0].expId},function(data){
		if(data.message=="0"){
			...
			window.location.href="<c:url value='/expqixinzhuan/Setting?expId="+rows[0].expId+"&showJsp=0&type=1'/>";
...
function doStart(){
	...
	$.messager.confirm('确认', '是否确定要开始活动吗？', function(r){
		...
		$.post(SITE_BASE_PATH+'expqixinzhuan/startExp?', {expId: ids.join(','),isBegin:"1"},function(data){
...
function doStop(){
	...
	$.messager.confirm('确认', '是否确定要结束活动吗？', function(r){
		...
		$.post(SITE_BASE_PATH+'expqixinzhuan/stopExp?', {expId: ids.join(','),isBegin:"0"},function(data){
			if(data.message=="success"){
				$('#mydatagrid').datagrid('reload');
				$.messager.show({
					title:'系统消息',
