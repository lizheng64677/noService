...
<body>
	    ...
	    <form id="dataForm"  method="post">
	    	...
	    	<tr>
	    		...
	    		<select name="showType" class="input" id="showType">
	    			<option value="">---请选择---</option>
  					<option value="0"  <c:if test="${expzhuan.showType ==0 }">selected="selected"</c:if>>应用下载</option>
  					<option value="1" <c:if test="${expzhuan.showType ==1 }">selected="selected"</c:if>>问卷调查</option>
	    		...
	    		<tr>
		    		<td>参与次数:</td>
		    		<td>
		    		<input type="text"  name="userTotal" value="${expzhuan.userTotal }" class="easyui-validatebox input"   placeholder="输入用户参与次数" missingMessage="输入用户参与次数" data-options="required:true" />
		    		</td>
	    		</tr>
	    		<tr>
	    		<td>固定收益:</td>
	    		<td>	
	    			<input type="text"  name="expUserGold" id="expUserGold" class="easyui-validatebox input"   placeholder="输入固定收益所得金币" missingMessage="请填固定收益所得金币" data-options="required:true" value="${expzhuan.expUserGold }" />
	    		</td>
	    		</tr>
	    		<tr>	    		
	    		<td>是否有标签:</td>
	    		...
	    		<td>上传的字段说明:</td>
	    		<td>	
	    			<input value="${expzhuan.tips }" maxlength="50" type="text"  name="tips" id="tips" class="easyui-validatebox input"   placeholder="上传的字段说明（APP下载需要）"  />
	    		</td>
	    		</tr>
	  	 		<tr>
	    		<td>是否有下载地址</td>
	    		<td>
	    			<c:if test="${expzhuan.isDown == 1}">
	    				<input type="radio" value="1"  name="isDown" class="easyui-validatebox radio" checked="checked" />
	    				是
	    				<input type="radio" value="0"  name="isDown" class="easyui-validatebox radio"  />
	    				否
	    			</c:if>
	    			<c:if test="${expzhuan.isDown == 0}">
	    				<input type="radio" value="1"  name="isDown" class="easyui-validatebox radio" />
	    				是
	    				<input type="radio" value="0"  name="isDown" class="easyui-validatebox radio" checked="checked" />
	    				否
	    			</c:if>		
	    		...
	    		<td>应用地址:</td>
	    		<td>	
	    			<input type="text"  name="expAppUrl" id="expAppUrl"  class="input"  placeholder="输入应用地址"  value="${expzhuan.expAppUrl }"/>
	    		</td>
	    		</tr>
	    		<tr>
	    		<td>活动图片:</td>
	...
	<script>
		...
		function submitForm(){
			...
			url="<c:url value='/expquanminzhuan/update'/>";
