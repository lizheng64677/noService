...
<script type="text/javascript">
	$(function() {
		...
		var expId=$("#expid").val();
		var url="<c:url value='/expprototype/findExpPrototype'/>";
		$.ajax({
			url : url,
			type : 'post',
			data : {expId:expId},
				...
				var html="<table cellpadding='3'>";
					$(data.result).each(function(i,result){
						...
						if(result.dictionary_type == 1){
							...
							if(!k){
								html+='<input type="radio" checked="checked" name='+result.dictionary_id+' onclick="checkRadio(this)" class="radioStyle chooseRadio"  tid="-1" value="-2"/><span>不限</span>';
							}else{
...
<script type="text/javascript">
	function assembleUserPrototype(){
		...
		var suburl="<c:url value='/expprototype/addExpPrototype'/>";
		//执行保存 
		$.ajax({
			url : suburl,
			type : 'post',
			dataType :'json',
			data:{paramdata:JSON.stringify(jsonArray),expId:expid,type:type},
			success : function(data) {
				if(data.message=="success"){
...
<body>
	<div class="easyui-panel" title="活动动态属性配置"   style="padding:10px;">
	    ...
	    <input type="hidden" value="${moduleType }" id="type"/>
