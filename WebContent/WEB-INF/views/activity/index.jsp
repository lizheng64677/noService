...
$(function() {
	$('#mydatagrid').datagrid({
		fitColumns : true,
		url : "<c:url value='/adminactivity/list'/>",
		method : "POST",
		pageSize :10, 
		pageList : [10,20,50],
		columns : [[
			{ "field":'id',checkbox:true },
					    ...
					    { "field": 'participantNumber',"title" : '参数人数',width:$(this).width() * 0.2},
					    { "field": 'visitTimes',"title" : '访问次数',width:$(this).width() * 0.2},
					    { "field": 'beginTime',"title" : '活动开始时间',width:$(this).width() * 0.2},
						{ "field": 'endTime',"title" : '活动结束时间',width:$(this).width() * 0.2},
...
function doAdd(){
	window.location.href="<c:url value='/adminactivity/jumpAdd'/>";
}
function doUpdate(){
	...
	window.location.href="<c:url value='/adminactivity/jumpEdit?id="+rows[0].id+"'/>";
}
function doDelete(){
	var rows=$("#mydatagrid").datagrid("getSelections");
	if(rows.length==0){
	...
	$.messager.confirm('确认', '是否确定要删除!', function(data){
		...
		$.post(SITE_BASE_PATH+'adminactivity/delete?', {id: ids.join(',')},function(data){
			if(data.result > 0){
