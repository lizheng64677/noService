		...
		columns : [[
			{ "field":'expDetailId',checkbox:true },
						...
						{ "field": 'proSet',"title":'操作',width:$(this).width()* 0.2,formatter:function(value,row,index){
							return "<a href='javascript:void(0)' id='addProId' onclick='getId()'>活动id</a>";
						}},
		]],
		toolbar :'#tool',
		pagination : true,
...
function getId(){
	...
	var expDetailId=rows[0].expDetailId;
	$.messager.alert("提示", "对应的活动id:"+expDetailId, true, "warning");
}
function doAdd(){
	window.location.href="<c:url value='/expdetail/jumpAdd?expId=${expId }'/>";
}
function doUpdate(){
	var rows=$("#mydatagrid").datagrid("getSelections");
	if(rows.length!=1){
	...
	window.location.href="<c:url value='/expdetail/jumpEdit?id="+rows[0].expDetailId+"'/>";
}
function doDelete(){
	var rows=$("#mydatagrid").datagrid("getSelections");
	if(rows.length==0){
