...
$(function() {
	$('#mydatagrid').datagrid({
		fitColumns : true,
		url : "<c:url value='/experience/list'/>",
		method : "POST",
		pageSize :10, 
		pageList : [10,20,50],
		columns : [[			    	
					    ...
					    { "field": 'showType',"title" : '抽奖类型',width:$(this).width() * 0.2,formatter:function(value,row){  
					    	if(value==0){
					    		return "红包方式"; 
					    	}else if(value==1){
					    		return "拼图方式"; 
...
function doAdd(){
	window.location.href="<c:url value='/experience/jumpAdd'/>";
}
...
function updataSeqNum(){
	...
	$.post(SITE_BASE_PATH+'experience/updataSeqNum?', {expId: rows[0].expId,seq:seq},function(data){
		if(data.message=="1"){
...
function doUpdate(){
	...
	$.post(SITE_BASE_PATH+'experience/isBeg?', {expId: rows[0].expId},function(data){
		if(data.message=="0"){
			
			window.location.href="<c:url value='/experience/jumpEdit?id="+rows[0].expId+"'/>";
...
function doDetailAdd(){
	...
	$.post(SITE_BASE_PATH+'experience/isBeg?', {expId: rows[0].expId},function(data){
// 	$.post(SITE_BASE_PATH+'experience/isBegProduct?', {expId: rows[0].expId},function(data){//一对一
		if(data.message=="0"){
...
function doStart(){
	...
	$.messager.confirm('确认', '是否确定要开始活动吗？', function(r){
		...
		$.post(SITE_BASE_PATH+'experience/startExp?', {expId: ids.join(','),isBegin:"1"},function(data){
...
function doStop(){
	...
	$.messager.confirm('确认', '是否确定要结束活动吗？', function(r){
		...
		$.post(SITE_BASE_PATH+'experience/stopExp?', {expId: ids.join(','),isBegin:"0"},function(data){
			if(data.message=="success"){
				$('#mydatagrid').datagrid('reload');
				$.messager.show({
					title:'系统消息',
...
function doSetting(){
	...
	$.post(SITE_BASE_PATH+'experience/isBeg?', {expId: rows[0].expId},function(data){
		if(data.message=="0"){
			...
			window.location.href="<c:url value='/experience/Setting?expId="+rows[0].expId+"&type=2'/>";
...
function doDelete(){
	...
	$.messager.confirm('确认', '是否确定要删除!', function(r){
		...
		$.post(SITE_BASE_PATH+'experience/delete?', {id: ids.join(',')},function(data){
			if(data.result > 0){
