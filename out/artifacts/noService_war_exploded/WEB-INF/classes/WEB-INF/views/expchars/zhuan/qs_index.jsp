...
$(function() {
	...
	$("#exper").bind("click",function(){
		...
		$.messager.confirm('确认', '是否确定导出数据!', function(data){
			...
			location.href="<c:url value='/expExcel/expTaskDataExport'/>?expType=1&clicentType="+$("#clientType").val()+"&beginTime="+$("#beginTime").val()+"&endTime="+$("#endTime").val();
	...
	$('#mydatagrid').datagrid({
		fitColumns : true,
		url : "<c:url value='/expChars/findZhuanList'/>",
		method : "POST",
		pageSize :10, 
		pageList : [10,20,50],
		columns : [[			    	
					    ...
					    { "field": 'userGold',"title" : '收益',width:$(this).width() * 0.2,formatter:function(value,row){
					    	return value+"金币";
					    }},
					    { "field": 'orderNum',"title" : '领取人数',width:$(this).width() * 0.2,formatter:function(value,row){
					    	return value+"人";
					    }},
					    { "field": 'tjNum',"title" : '提交人数',width:$(this).width() * 0.2,formatter:function(value,row){ 
					    	return value+"人";
						}},
					    { "field": 'shNum',"title" : '审核通过人数',width:$(this).width() * 0.2,formatter:function(value,row){
					    	return value+"人";
					    }},
